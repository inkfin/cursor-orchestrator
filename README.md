...
(`.cursor-plugin/plugin.json` plus default `agents/`, `rules/`, `skills/`, and `hooks/`).

`marketplace.json` with `"source": "./"` is included for team marketplace import from this repo (verified loadable).

## How to install

### GitHub / team marketplace

Import this repository via your team's Cursor marketplace (admin **Import from Repo**). Requires a git remote Cursor can reach (typically GitHub).
...
```bash
agent --plugin-dir /path/to/cursor-orchestrator ...
```
...
| Team Marketplace | Admin Import from Repo; `marketplace.json` with `source: "./"` |
